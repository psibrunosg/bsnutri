...
describe('NutritionWorkspace editor modes', () => {
  ...
  it('edita gramas e troca alimento sem remover item', async () => {
    ...
    expect(monday).toHaveAttribute('aria-controls', 'plan-day-panel')
    expect(screen.getByRole('tabpanel', { name: 'Segunda-feira' })).toHaveAttribute('id', 'plan-day-panel')
    expect(screen.getByRole('tabpanel', { name: 'Segunda-feira' })).toHaveAttribute('aria-labelledby', monday.getAttribute('id'))
    ...
    expect(await within(breakfast).findByRole('option', { name: 'Arroz' })).toBeInTheDocument()
    expect(within(breakfast).queryByRole('option', { name: 'Batata' })).not.toBeInTheDocument()
  ...
  it('mostra e oculta a lista de compras agregada por alimento', async () => {
    ...
    const breakfast = screen.getByRole('region', { name: 'Café da manhã' })
    fireEvent.change(within(breakfast).getByLabelText('Buscar alimento no Café da manhã'), { target: { value: 'arr' } })
    await within(breakfast).findByRole('option', { name: 'Arroz' })
    fireEvent.change(within(breakfast).getByLabelText('Alimento no Café da manhã'), { target: { value: 'food-1' } })
