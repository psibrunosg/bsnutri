...
function Dashboard({ session, workspace }: { session: Session; workspace: Workspace }) {
  const [patients, setPatients] = useState<Patient[]>([]); const [query, setQuery] = useState(''); const [open, setOpen] = useState(false); const [selected, setSelected] = useState<Patient | null>(null); const [menu, setMenu] = useState(false); const [error, setError] = useState('')
  ...
  return <div className="app-shell"><aside className={menu ? 'sidebar open' : 'sidebar'}><div className="brand inverse"><span>BS</span><strong>BSNutri</strong><button aria-label="Fechar menu" onClick={() => setMenu(false)}><X/></button></div><nav><a className="active"><Users/>Pacientes</a><a><Utensils/>Planos alimentares</a><a><CalendarDays/>Agenda</a></nav><button className="logout" onClick={() => supabase.auth.signOut()}><LogOut/>Sair</button></aside>
    <main className="content"><header><button className="menu-button" aria-label="Abrir menu" onClick={() => setMenu(true)}><Menu/></button><div><small>{workspace.organizations?.name}</small><h1>{selected ? 'Prontuário nutricional' : 'Pacientes'}</h1></div>{!selected && <button className="primary compact" onClick={() => setOpen(true)}><Plus/>Novo paciente</button>}</header>
      {selected ? <PatientDetail patient={selected} session={session} workspace={workspace} onBack={() => setSelected(null)}/> : <>
