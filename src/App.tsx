...
export function App() {
  ...
  if (loading) return <main className="loading">Carregando BSNutri...</main>
  if (!session) return <AuthScreen/>
  if (patientAccess) return <PatientPortal patient={patientAccess}/>
  if (!ready || !workspace) return <Bootstrap session={session} onReady={() => loadWorkspace(session)}/>
