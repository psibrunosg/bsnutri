...
import { CalendarDays, Dumbbell, LogOut, Menu, Plus, Search, Users, Utensils, X } from 'lucide-react'
...
function Dashboard({ session, workspace }: { session: Session; workspace: Workspace }) {
  const [patients, setPatients] = useState<Patient[]>([]); const [page, setPage] = useState<'patients' | 'nutrition' | 'care' | 'exercises'>('patients'); const [query, setQuery] = useState(''); const [open, setOpen] = useState(false); const [selected, setSelected] = useState<Patient | null>(null); const [menu, setMenu] = useState(false); const [error, setError] = useState('')
