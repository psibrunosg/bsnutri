...
export function PatientDetail({ patient, session, workspace, onBack }: { patient: Patient; session: Session; workspace: Workspace; onBack: () => void }) {
  ...
  const load = useCallback(async () => {
    const [assessmentResult, measurementResult, consentResult, labResult, templateResult, assignmentResult, summaryResult] = await Promise.all([
    ...
    const first = assessmentResult.error ?? measurementResult.error ?? consentResult.error ?? labResult.error ?? templateResult.error ?? assignmentResult.error ?? summaryResult.error
