...
type LabResult = { id: string; collected_on: string; test_name: string; result_value: number | null; unit: string | null; reference_range: string | null; notes: string | null; attachment_name:string|null; attachment_url:string|null }
...
export function PatientDetail({ patient, session, workspace, onBack }: { patient: Patient; session: Session; workspace: Workspace; onBack: () => void }) {
  ...
  const load = useCallback(async () => {
    const [assessmentResult, measurementResult, consentResult, labResult, templateResult, assignmentResult, summaryResult, goalResult, weeklyResult] = await Promise.all([
      ...
      supabase.from('lab_results').select('id,collected_on,test_name,result_value,unit,reference_range,notes,attachment_name,attachment_url').eq('patient_id', patient.id).order('collected_on', { ascending: false }),
  ...
  async function addLab(event: FormEvent<HTMLFormElement>) {
    ...
    const attachmentUrl=String(data.get('attachmentUrl')).trim(),attachmentName=String(data.get('attachmentName')).trim()
    if(Boolean(attachmentUrl)!==Boolean(attachmentName))return setError('Informe nome e link do anexo juntos.')
    const { error } = await supabase.from('lab_results').insert({ organization_id: workspace.organization_id, patient_id: patient.id, collected_on: String(data.get('collectedOn')), test_name: String(data.get('testName')), result_value: resultValue, unit: String(data.get('unit')) || null, reference_range: String(data.get('referenceRange')) || null, notes: String(data.get('notes')) || null, attachment_name:attachmentName||null,attachment_url:attachmentUrl||null, created_by: session.user.id })
    if (error) setError(error.message); else { event.currentTarget.reset(); await load() }
  }
  ...
  return <section className="detail"><button className="back" onClick={onBack}>Voltar para pacientes</button><div className="detail-heading"><div className="avatar">{patient.full_name.slice(0, 2).toUpperCase()}</div><div><small>{patient.anonymous_code}</small><h1>{patient.full_name}</h1></div><button className="secondary" onClick={()=>void exportSummary()}>Exportar resumo</button></div>{error && <div className="notice error" role="alert">{error}</div>}
    ...
    <DetailSection title="Exames basicos"><form onSubmit={addLab}><div className="field-row"><label>Data da coleta<input name="collectedOn" type="date" required/></label><label>Exame<input name="testName" required maxLength={160}/></label></div><div className="field-row"><label>Resultado<input name="value" inputMode="decimal"/></label><label>Unidade<input name="unit" maxLength={40}/></label></div><label>Referencia<input name="referenceRange" maxLength={120}/></label><div className="field-row"><label>Nome do anexo<input name="attachmentName" maxLength={180} placeholder="Ex.: hemograma.pdf"/></label><label>Link seguro do anexo<input name="attachmentUrl" type="url" placeholder="https://..."/></label></div><label>Observacao<textarea name="notes" maxLength={1000}/></label><button className="primary">Salvar exame</button></form>{labs.length ? labs.map(l => <article className="history" key={l.id}><time>{new Date(`${l.collected_on}T12:00:00`).toLocaleDateString('pt-BR')}</time><strong>{l.test_name}</strong><p>{l.result_value ?? 'Sem resultado numerico'}{l.unit ? ` ${l.unit}` : ''}{l.reference_range ? ` · Referencia: ${l.reference_range}` : ''}</p>{l.attachment_url&&<p><a href={l.attachment_url} target="_blank" rel="noreferrer">{l.attachment_name}</a></p>}{l.notes && <p>{l.notes}</p>}</article>) : <p className="muted">Nenhum exame registrado.</p>}</DetailSection></div>
