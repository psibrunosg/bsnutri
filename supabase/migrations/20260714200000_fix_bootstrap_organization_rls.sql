-- bootstrap_organization as security invoker depends on RLS on public.organizations
-- allowing the row through. Re-affirm the policy/grants explicitly (idempotent) and
-- restore the "already has an active organization" guard that was dropped when the
-- membership insert moved into the organizations_bootstrap_owner trigger.
